"use client";

import { Label } from "@/components/ui/label";

/**
 * A reusable TextField component with support for:
 * - React Hook Form integration (field + validation errors)
 * - Custom label
 * - Leading icon
 * - Controlled/uncontrolled usage
 *
 * @param {object} props
 * @param {object} [props.field] - RHF field object (`Controller` field)
 * @param {string} [props.value] - Value for controlled usage (if not using RHF)
 * @param {function} [props.onChange] - Change handler for controlled usage
 * @param {string} [props.type] - Input type, defaults to "text"
 * @param {React.ComponentType} [props.Icon] - Optional leading icon
 * @param {string} [props.label] - Label text
 * @param {string} [props.error] - Validation error message (from RHF or manual)
 * @param {string} [props.className] - Additional Tailwind classes
 * @param {object} [props.rest] - Additional props passed to Input
 */
export default function TextField({
  field,
  value,
  onChange,
  type = "text",
  Icon,
  label,
  error,
  className = "",
  ...rest
}) {
  const val = field ? field.value : value;
  const handleChange = field ? field.onChange : onChange;

  return (
    <div className="w-full">
      {label && (
        <Label className="mb-1 block text-sm font-[300] text-gray-700">
          {label}
        </Label>
      )}

      <div className="relative">
        {Icon && (
          <Icon className="absolute left-0 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400 pointer-events-none" />
        )}
        <input
          type={type}
          value={val}
          onChange={handleChange}
          {...(field ? field : {})}
          {...rest}
          className={`w-full bg-transparent border-0 border-b border-black rounded-none py-2 px-0 focus:outline-none focus:ring-0 transition-colors duration-200 ${className}`}
        />
      </div>

      {error && <p className="mt-1 text-sm text-red-500">{error}</p>}
    </div>
  );
}
